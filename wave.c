...
// function displayWAVdata calculate 1-sec samples
void displayWAVdata(short int d[]){
	int i, j;
	double sum200, rms200, max200=0.0, min200=200;
	double Leqf[8], sum2000 = 0.0;
	
	for(i=0; i<80; i++){
		sum200=0.0;	// initialize the accumulate
		for(j=0; j<SAMPLE_RATE/80; ++j){
			sum200 += (*d)*(*d);
			d++;	// traet d as a pointer, pointer incresement
		}
			sum2000 += sum200;
		if(i%10 == 9){	// for every 10 pieces of rms200, we get a rms200
	...
	printf("\nmin = %.2f, max = %.2f\n", min200, max200);
#endif
#ifdef COMM	// only in the case COMM is defined, send data to the server
	send_data_curl(Leqf);
#endif
}
